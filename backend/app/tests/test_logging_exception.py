...
def test_global_exception_handler_unhandled(client, caplog):
    ...
    assert any(
        "Unhandled exception occurred while processing GET /api/test-logging-exception/error" in msg
...
def test_global_exception_handler_http(client, caplog):
    ...
    log_messages = [record.message for record in caplog.records]
    assert any(
        "HTTP exception on GET /api/test-logging-exception/http-error: status_code=400 detail=Simulated client error"
        in msg
        for msg in log_messages
    )
